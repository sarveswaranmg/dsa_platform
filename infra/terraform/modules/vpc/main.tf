# VPC with public + private subnets across N AZs, one (or one-per-AZ) NAT
# gateway for private-subnet egress (pulling images, talking to AWS APIs).
...
locals {
  ...
  # /20 per subnet out of a /16 VPC: public subnets in the low half, private
  # in the high half, so the two tiers never overlap regardless of az_count.
