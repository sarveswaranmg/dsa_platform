...
def get_object_bytes(key: str) -> bytes:
    """Server-side fetch (direct, not presigned) — used by the ingestion job
    to read the uploaded resume, never from a request path."""
    settings = get_settings()
    body = _client(settings.s3_endpoint_url).get_object(Bucket=settings.s3_bucket, Key=key)[
        "Body"
    ]
    return body.read()
...
def ensure_bucket() -> None:
    ...
    # The examiner console PUTs resume files straight to S3 from the
